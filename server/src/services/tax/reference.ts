...
async function getCachedReference(companyId: string, db: TaxReferenceDb): Promise<TaxReadinessDto> {
  return db.$transaction(async (tx) => {
    ...
    const salesTaxCodes = refreshFailed ? [] : taxCodesForReadiness(codeRows as TaxCodeRow[], 'sales');
    const salesReadiness = cachedSalesTaxReadiness(
      company.taxUsingSalesTax,
      codeRows as TaxCodeRow[],
    ...
    return {
      ...
      taxCodes: taxCodesForReadiness(codeRows as TaxCodeRow[], 'purchase'),
